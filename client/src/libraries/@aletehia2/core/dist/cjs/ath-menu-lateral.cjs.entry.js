...
const AthMenuLateral = class {
    ...
    generateMenuLateralItems() {
        ...
        this.generatedMenuLateralItems = items.map((item, index) => ({
            ...
            type: item.href != '' ? menuLateral_model.MenuLateralItemType.Link : menuLateral_model.MenuLateralItemType.Action,
        }));
    }
    getItems() {
        if (this.items) {
            ...
            return {
                ...
                ...(isLink
                    ? {
                        ...
                        rel: item.rel,
                        target: item.target,
                        type: menuLateral_model.MenuLateralItemType.Link,
                    }
                    : {
                        type: menuLateral_model.MenuLateralItemType.Action,
                    }),
            };
        });
    }
    ...
    renderIcon = item => index.h("ath-icon", { icon: item.icon, color: item.disabled ? 'disabled' : item.selected ? 'inverse' : 'primary', size: "sm" });
    // Interaction
        ...
        switch (event.key) {
            ...
            case ' ':
                if (index !== -1) {
                    ...
                    if (item.type === menuLateral_model.MenuLateralItemType.Link && item.href && !item.disabled) {
    ...
    // Render
    render() {
        return (index.h(index.Host, { key: '8546b332f533e29fb44a544731c06200416c0e22' }, index.h("div", { key: 'b8fb9cd974b1699eeec867f7db444373c76a9879', role: "menubar", class: "ath-menu-lateral", tabindex: "0", "aria-activedescendant": this.focusedItemId, onKeyDown: e => this.handleMenuItemKeyDown(e), onFocus: this.handleFocus, onBlur: () => {
                this.showFocused = false;
            } }, this.generatedMenuLateralItems.map(item => (index.h("div", { class: "ath-menu-lateral__item-wrapper" }, index.h("div", { class: "ath-menu-lateral__item" }, item.badgeValue ? (index.h("ath-badge", { value: item.badgeValue, max: item.badgeMax, position: "top-right", "distance-x": -5, "distance-y": -4, color: item.disabled ? 'disabled' : 'accent' }, this.renderMenuItem(item))) : (this.renderMenuItem(item))), index.h("span", { class: ((this.hoveredItemId === item.id && !item.disabled) || this.focusedItemId === item.id) && item.tooltipText ? '' : 'ath-visibility-hidden' }, index.h(fcHelpDescription.FcHelpDescription, { text: item.tooltipText, position: "bottom-right" }))))))));
    }
...
exports.ath_menu_lateral = AthMenuLateral;
//# sourceMappingURL=ath-menu-lateral.entry.cjs.js.map
