...
const FcPageButton = props => {
    ...
    return (index.h("button", { ref: props.pageButtonRef, class: { ...getPageButtonClass() }, onClick: handleClick, "aria-label": `Página ${props.pageNumber}`, disabled: props.disabled, "aria-disabled": props.disabled ? 'true' : 'false', "aria-current": props.selected ? 'page' : undefined }, index.h("span", { class: "ath-pagination-page-button-number" }, props.pageNumber)));
};
...
const AthPagination = class {
    constructor(hostRef) {
        ...
        this.athPaginate = index.createEvent(this, "athPaginate");
        this.athItemsPerPageChange = index.createEvent(this, "athItemsPerPageChange");
    }
    ...
    generateNavigationButtons() {
        return (index.h("div", { class: "ath-pagination-page-control" }, index.h("ul", { class: "ath-pagination-page-control-list" }, !this.noEndButtons && (index.h("li", null, index.h("ath-button", { size: "sm", color: "secondary", "icon-position": "icon-only", icon: "first", clear: true, onAthClick: () => this.onAthPaginate(1), disabled: this.disabled || this.currentPageState === 1, "aria-label": "Primera p\u00E1gina" }))), index.h("li", null, index.h("ath-button", { size: "sm", color: "secondary", "icon-position": "icon-only", icon: "chevron_left", clear: true, onAthClick: () => this.onAthPaginate(this.currentPageState - 1), disabled: this.disabled || this.currentPageState <= 1, "aria-label": "Anterior" })), !this.noJumpButtons && this.currentPageState > 1 && this.getBlockStart(this.currentPageState, this.paginationSize) > 1 && (index.h("li", null, index.h("ath-button", { size: "sm", color: "secondary", "icon-position": "icon-only", icon: "menu_horizontal", clear: true, onAthClick: () => this.onPrevBlock(), disabled: this.disabled, "aria-label": "Bloque anterior" }))), this.getPaginationBlock(this.getBlockStart(this.currentPageState, this.paginationSize), this.totalPages, this.paginationSize).map(page => (index.h("li", null, index.h(FcPageButton, { ...this.getPageButtonProps(page), pageButtonRef: (el) => {
                if (el)
                    this.pageButtonRefs[page] = el;
            }, onClick: () => this.onAthPaginate(page) })))), !this.noJumpButtons &&
            this.currentPageState < this.totalPages &&
            this.getBlockStart(this.currentPageState, this.paginationSize) + this.paginationSize <= this.totalPages && (index.h("li", null, index.h("ath-button", { size: "sm", color: "secondary", "icon-position": "icon-only", icon: "menu_horizontal", clear: true, onAthClick: () => this.onNextBlock(), disabled: this.disabled, "aria-label": "Bloque siguiente" }))), index.h("li", null, index.h("ath-button", { size: "sm", color: "secondary", "icon-position": "icon-only", icon: "chevron_right", clear: true, onAthClick: () => this.onAthPaginate(this.currentPageState + 1), disabled: this.disabled || this.currentPageState === this.totalPages, "aria-label": "Siguiente" })), !this.noEndButtons && (index.h("li", null, index.h("ath-button", { size: "sm", color: "secondary", "icon-position": "icon-only", icon: "last", clear: true, onAthClick: () => this.onAthPaginate(this.totalPages), disabled: this.disabled || this.currentPageState === this.totalPages, "aria-label": "\u00DAltima p\u00E1gina" }))))));
    }
    generateItemsCount() {
        if (!this.noItemsCount && !this.isMobileScreen) {
            return (index.h("div", { class: "ath-pagination-items-count" }, index.h("span", null, (this.currentPageState - 1) * this.selectedItemsPerPage + 1, " - ", Math.min(this.currentPageState * this.selectedItemsPerPage, this.totalItems), " de ", this.totalItems, ' ', "elementos")));
        }
    }
    generateItemsSelector() {
        if (!this.noItemsSelector && !this.isMobileScreen) {
            return (index.h("div", { class: "ath-pagination-items-selector" }, index.h("ath-dropdown", { onAthChange: e => this.onItemsPerPageChange(e.detail), disabled: this.disabled }, this.itemsSelectorArray.map(items => (index.h("ath-dropdown-option", { text: `${items} elementos por página`, value: items.toString(), selected: items === this.selectedItemsPerPage }))))));
        }
    }
    getHostAttributes = () => {
        return {
    ...
    generateStructure() {
        return (index.h("nav", { class: "ath-pagination", ...this.getHostAttributes() }, this.generateNavigationButtons(), this.generateItemsCount(), this.generateItemsSelector()));
    }
    ...
    render() {
        return index.h(index.Host, { key: 'bfd95602e0e7278c80591b6fc9fd9a0a2da24d3f' }, this.generateStructure());
    }
...
exports.ath_pagination = AthPagination;
//# sourceMappingURL=ath-pagination.entry.cjs.js.map
