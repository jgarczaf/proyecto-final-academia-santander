...
const AthMenuHorizontal = class {
    ...
    generateItems() {
        ...
        this.menuHorizontalItems = items.map((item, i) => ({
            ...
            target: !!item.target ? item.target : menuHorizontal_model.TargetType.Self,
            value: item.value,
        }));
    }
    ...
    renderLabel = item => {
        if (item.badgeValue) {
            return (index.h("ath-badge", { label: item.badgeLabel, position: "right", value: item.badgeValue, max: item.badgeMax, color: item.disabled ? 'disabled' : 'accent' }, item.label));
        }
        else
            return item.label;
    };
    render() {
        return (index.h(index.Host, { key: 'e604afad2f73baed70a7cf3f4b052d9907dfbbe0' }, index.h("div", { key: 'be2c2e45191e8e6db827065f50d4b8a13792d155', class: this.getHostClassNames() }, this.showArrows && !this.isLeftArrowHidden && (index.h("div", { key: 'df042365d7a1af0f831f29befb02f28a7d9399d3', class: "ath-menu-horizontal__button ath-menu-horizontal__button-left", "aria-hidden": "true" }, index.h(fcButtonComp.FcButtonComp, { key: '6365524247d274beffac5f7e1a1f6e0942ddb517', icon: "chevron_left", color: "default", iconPosition: button_model.ButtonIconPosition.IconOnly, size: icons.IconSize.Small, onClick: () => this.scrollItems('left'), "aria-hidden": "true", tabindex: -1 }))), index.h("nav", { key: '9d595f5b1f58da4e3e2a5193343c5f11ec45b41a', class: "ath-menu-horizontal--item-group", ref: el => (this.itemGroup = el), onScroll: () => this.updateArrowsVisibility() }, index.h("ul", { key: '42146685284d907fa5e1b51985bf66973ff15d9e', class: "ath-menu-horizontal--item-group--list", role: "menubar", "aria-label": this.athAriaLabel }, this.menuHorizontalItems.map((item, i) => (index.h("li", { class: {
                'last-item--padding-right': i === this.menuHorizontalItems.length - 1,
                'ath-menu-horizontal--item-group--list--item': true,
            }, role: "none" }, index.h("a", { role: "menuitem", id: item.id, href: item.href, target: `_${item.target}`, "aria-disabled": item.disabled ? 'true' : 'false', tabindex: item.id === this.selectedMenuHorizontalItemId ? '0' : '-1', "aria-current": item.id === this.selectedMenuHorizontalItemId ? 'page' : undefined, class: this.getMenuHorizontalItemItemClassNames(item), onClick: e => this.handleItemClick(e, item), onKeyDown: event => this.handleKeyDown(event, item), onFocus: this.handleItemFocus }, index.h("div", { class: "ath-menu-horizontal-item--content" }, index.h("span", { class: "ath-menu-horizontal-label" }, this.renderLabel(item)), item.id === this.selectedMenuHorizontalItemId && item.id === this.activeMenuHorizontalItemId && index.h("span", { class: "sr-only" }, " seleccionado")), item.target === 'blank' && index.h("span", { class: "sr-only ath-external-label" }, ' ' + item.externalLabel), index.h("div", { class: { 'ath-menu-horizontal-item--indicator': this.selectedMenuHorizontalItemId === item.id } }))))))), this.showArrows && !this.isRightArrowHidden && (index.h("div", { key: '887898198af9b634e36fae391d82641b128baa63', class: "ath-menu-horizontal__button ath-menu-horizontal__button-right", "aria-hidden": "true" }, index.h(fcButtonComp.FcButtonComp, { key: '264f27a99fd4f3ded1cba0ca677bdfc064255f7e', icon: "chevron_right", color: "default", iconPosition: button_model.ButtonIconPosition.IconOnly, size: icons.IconSize.Small, onClick: () => this.scrollItems('right'), buttonAriaLabel: "Mostrar m\u00E1s pesta\u00F1as hacia la derecha", "aria-hidden": "true", tabindex: -1 }))))));
    }
...
exports.ath_menu_horizontal = AthMenuHorizontal;
//# sourceMappingURL=ath-menu-horizontal.entry.cjs.js.map
