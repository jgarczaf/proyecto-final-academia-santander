...
const AthMenuVertical = class {
    ...
    renderMenu() {
        ...
        this.menuItems.forEach(item => {
            ...
            if (this.appearance === MenuVerticalAppearances.Primary) {
                ...
                menuArray.push(index.h("li", { class: "ath-section-sublist" }, index.h("ath-section-title", { headingText: item.text, color: "accent" }), Array.from(item.children).length > 0 && index.h("ul", { class: "ath-menu-vertical__menu" }, this.createMenu(itemId, Array.from(item.children), 2))));
            }
        });
        return menuArray;
    }
    renderLevel1(item, id) {
        ...
        if (item.tagName === 'ath-MENU-VERTICAL-ITEM-LINK') {
            return (index.h("li", { role: "none" }, index.h("a", { role: "menuitem", id: id, ...(item.disabled ? {} : this.getLinkProp(item)), class: renderLevel1ClassNames, tabIndex: this.getTabIndex(item), onFocus: item.disabled ? undefined : () => this.setFocusedItem(id), "aria-disabled": item.disabled ? 'true' : undefined, "aria-label": item.ariaLabel, "aria-labelledby": item.ariaLabelledby, "aria-current": item.selected ? 'page' : undefined, onClick: e => {
                    if (item.disabled) {
                        e.preventDefault();
                        return;
                    }
                    this.handleClick(id);
                }, onKeyDown: item.disabled ? undefined : e => this.handleKeyDown(e, id, 1, false), ref: el => el && this.registerFocusableItem(el) }, index.h("div", { class: "ath-menu-vertical__level-1__icon" }, index.h("ath-icon", { icon: !!item.icon ? item.icon : 'placeholder', size: "lg", color: "inherit" })), index.h("div", { class: "ath-menu-vertical__level-1__text" }, item.text))));
        }
        return (index.h("li", { class: "ath-menu-vertical__list__group", role: "none" }, index.h("div", { id: id, class: renderLevel1ClassNames, role: "menuitem", tabIndex: item.disabled ? -1 : this.focusedItemId === id ? 0 : -1, onFocus: item.disabled ? undefined : () => this.setFocusedItem(id), "aria-label": item.ariaLabel, "aria-controls": `${id}-list`, "aria-expanded": Array.from(item.children).length > 0 ? (isOpen ? 'true' : 'false') : undefined, "aria-haspopup": Array.from(item.children).length > 0 ? 'menu' : undefined, "aria-current": Array.from(item.children).length === 0 && item.selected ? 'page' : undefined, "aria-disabled": item.disabled ? 'true' : undefined, onClick: item.disabled || isOpen ? undefined : Array.from(item.children).length > 0 ? () => this.handleClickOpen(id, 1, false) : () => this.handleClick(id), onKeyDown: item.disabled ? undefined : e => this.handleKeyDown(e, id, 1, Array.from(item.children).length > 0), ref: el => el && this.registerFocusableItem(el) }, index.h("div", { class: "ath-menu-vertical__level-1__icon" }, index.h("ath-icon", { icon: !!item.icon ? item.icon : 'placeholder', size: "lg", color: "inherit" })), index.h("div", { class: "ath-menu-vertical__level-1__text" }, index.h("span", null, item.text), Array.from(item.children).length > 0 && (index.h("div", { class: `ath-menu-vertical__level-1__text__button${isOpen ? ' open' : ''}` }, index.h("ath-icon", { size: "md", icon: "chevron_down", color: "inherit", "aria-hidden": "true" }))))), Array.from(item.children).length > 0 && isOpen && (index.h("ul", { role: "menu", id: `${id}-list`, class: "ath-menu-vertical__menu ath-menu-vertical__sublist" }, this.createMenu(id, Array.from(item.children), 2)))));
    }
    createMenu(id, arrayMenu, level) {
        ...
        arrayMenu.forEach(item => {
            ...
            item.id = itemId;
            menuArray.push(index.h("li", { role: "none", class: "ath-menu-vertical__sublist" }, index.h("div", { class: menuClassNames }, item.tagName === 'ath-MENU-VERTICAL-ITEM-LINK' ? this.createLink(item, level) : this.createAction(item, hasChildren, itemId, isOpen, level)), hasChildren && isOpen && (index.h("ul", { role: "menu", id: `${itemId}-list`, class: "ath-menu-vertical__menu ath-menu-vertical__sublist" }, this.createMenu(itemId, Array.from(item.children), level + 1)))));
        });
        return menuArray;
    }
    createAction(item, hasChildren, itemId, isOpen, level) {
        ...
        const actionClassNames = [
            ...
            .filter(Boolean)
            .join(' ');
        return (index.h("div", { role: "menuitem", id: itemId, class: actionClassNames, tabIndex: this.getTabIndex(item), onFocus: item.disabled ? undefined : () => this.setFocusedItem(itemId), "aria-label": item.ariaLabel, "aria-controls": hasChildren ? `${itemId}-list` : item.ariaControls, "aria-expanded": Array.from(item.children).length > 0 ? (isOpen ? 'true' : 'false') : undefined, "aria-haspopup": hasChildren ? 'menu' : undefined, "aria-current": Array.from(item.children).length === 0 && item.selected ? 'page' : undefined, "aria-disabled": item.disabled ? 'true' : undefined, onClick: item.disabled ? undefined : hasChildren ? () => this.handleClickOpen(itemId, level, true) : () => this.handleClick(itemId), onKeyDown: item.disabled ? undefined : e => this.handleKeyDown(e, itemId, level, hasChildren), ref: el => el && this.registerFocusableItem(el) }, index.h("span", null, item.text), hasChildren && (index.h("span", { class: `ath-menu-vertical__wrapper-title__button${isOpen ? ' open' : ''}` }, index.h("ath-icon", { size: "md", icon: "chevron_down", color: "inherit", "aria-hidden": "true" })))));
    }
    createLink(item, level) {
        ...
        const linkClassNames = [
            ...
            .filter(Boolean)
            .join(' ');
        return (index.h("a", { role: "menuitem", id: item.id, ...linkProps, class: linkClassNames, tabIndex: this.getTabIndex(item), onFocus: item.disabled ? undefined : () => this.setFocusedItem(item.id), "aria-disabled": item.disabled ? 'true' : undefined, "aria-label": item.ariaLabel ? item.ariaLabel : undefined, "aria-labelledby": item.ariaLabelledby ? item.ariaLabelledby : undefined, "aria-current": item.selected ? 'page' : undefined, onClick: e => {
                if (item.disabled) {
                    e.preventDefault();
                    return;
                }
                this.handleClick(item.id);
            }, onKeyDown: item.disabled ? undefined : e => this.handleKeyDown(e, item.id, 3, false), ref: el => el && this.registerFocusableItem(el) }, index.h("span", null, item.text), item.target === 'blank' && index.h("span", { class: "ath-visibility-hidden" }, item.externalLabel)));
    }
    getTabIndex(item) {
        if (item.disabled)
            return -1;
    ...
    render() {
        return (index.h(index.Host, { key: 'd4f9daeec69d54582f4fc2d4bc9421dcfcee9ab8', role: "navigation", class: "ath-menu-vertical" }, index.h("ul", { key: '6afec765afcb518ba4c2409e5304dc9822c4cab7', role: "menu", class: this.getListClassNames() }, this.renderMenu())));
    }
};
AthMenuVertical.style = menuVerticalCss;

exports.ath_menu_vertical = AthMenuVertical;
//# sourceMappingURL=ath-menu-vertical.entry.cjs.js.map
